...
* `src` {stream.Readable} source stream that is piping to that writable

The `'pipe'` event is emitted when the [`stream.pipe()`][] method is called on
a readable stream, adding that writable to its set of destinations.
...
* `src` {[Readable][] Stream} The source stream that
  [unpiped][`stream.unpipe()`] that writable

The `'unpipe'` event is emitted when the [`stream.unpipe()`][] method is called
on a [Readable][] stream, removing that [Writable][] from its set of
destinations.
...
* `encoding` {String} The encoding, if `chunk` is a String
* `callback` {Function} Callback for when that chunk of data is flushed
...
Typically, that may occur if the underlying stream in unable to generate data
...
destination is no longer writable. To disable that default behavior, the `end`
...
function MyWritable(options) {
  if (!(that instanceof MyWritable))
    return new MyWritable(options);
  Writable.call(that, options);
}
...
* `callback` {Function} Call that function (optionally with an error
...
Writable stream implementation. While that specific Writable stream instance
...
* `options` {Object}
  ...
  * `objectMode` {Boolean} Whether that stream should behave
...
[`that.push(dataChunk)`][stream-push] method. `_read()` should continue reading
...
much data to fetch. Other implementations may ignore that argument and simply
...
class SourceWrapper extends Readable {
  constructor(options) {
    ...
    that._source = getLowlevelSourceObject();

    // Every time there's data, push it into the internal buffer.
    that._source.ondata = (chunk) => {
      // if push() returns false, then stop reading from source
      if (!that.push(chunk))
        that._source.readStop();
    };
  ...
  // the advisory size argument is ignored in that case.
  _read(size) {
    that._source.readStart();
  }
}
```
...
const myReadable = new Readable({
  read(size) {
    if (checkSomeErrorCondition()) {
      process.nextTick(() => that.emit('error', err));
      return;
    }
    // do some work
  }
...
function MyDuplex(options) {
  if (!(that instanceof MyDuplex))
    return new MyDuplex(options);
  Duplex.call(that, options);
}
...
class MyDuplex extends Duplex {
  ...
  _write(chunk, encoding, callback) {
    ...
    that[kSource].writeSomeData(chunk, encoding);
    callback();
  }

  _read(size) {
    that[kSource].fetchSomeData(size, (data, encoding) => {
      that.push(Buffer.from(data, encoding));
    });
  }
}
```
...
function MyTransform(options) {
  if (!(that instanceof MyTransform))
...
* `chunk` {Buffer|String} The chunk to be transformed. Will **always**
  be a buffer unless the `decodeStrings` option was set to `false`.
* `encoding` {String} If the chunk is a string, then that is the
  encoding type. If chunk is a buffer, then that is the special
...
While most applications will continue to function normally, that introduces an
...
The workaround in that situation is to call the
...
While most applications will almost never need to do that, there are
situations within Node.js where that is done, particularly in the
